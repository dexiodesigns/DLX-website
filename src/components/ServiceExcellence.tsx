        ...
        >
          ...
          {Object.values(TAB_CONFIGS).map((tabConfig) => (
            <button
              key={tabConfig.label}
              ref={(el) => {
                tabRefs.current[tabConfig.label] = el;
              }}
              onClick={() => setActiveTab(tabConfig.label)}
              className={`px-4 md:px-8 py-2 md:py-2.5 rounded-[20px] text-[14px] md:text-base font-medium leading-6 capitalize whitespace-nowrap relative z-10 transition-colors duration-300 ${
          ...
          <AnimatePresence mode="wait">
            <motion.div
              ...
              initial={{
                opacity: 0,
                x:
                  tabOrder.indexOf(activeTab) > tabOrder.indexOf(activeTab)
                    ? 20
                    : -20,
                y: 0,
              }}
            ...
            >
              <h2
                className="text-[28px] md:text-[40px] font-semibold leading-tight mb-6 font-sora md:font-sans"
                style={{ color: "#F1F2F4", fontFamily: "Sora, sans-serif" }}
              >
                {getTabConfig(activeTab).content.title}
              </h2>
              <p
                className="text-sm md:text-base leading-relaxed mb-8 max-w-[38rem] mx-auto md:mx-0"
                style={{ color: "#D5D7DD" }}
              >
              ...
              {activeTab === "Services" && (
                <div className="flex justify-center md:justify-start gap-8 md:gap-12 mb-8">
                  <div className="flex flex-col">
                    <span
                      className="text-3xl md:text-4xl font-bold mb-1 font-sora"
                      style={{ fontFamily: "Sora, sans-serif" }}
                    >
                      100+
                    </span>
                    <span
                      className="font-sora text-xs md:text-base font-normal leading-6 tracking-[1px]"
                      style={{ color: "#D5D7DD" }}
                    >
                      Projects
                    </span>
